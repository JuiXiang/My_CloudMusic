...
const fetchSongDetail = async (id) => {
  if (!id) return;
  try {
    const res = await api.get("/song/detail", { params: { ids: id } });
    //   console.log(res)
    const detail = (res.songs || [])[0];
    if (!detail) return;
    songTitle.value = detail.name || "未知歌曲";
    songArtist.value = (detail.ar || detail.artist)
      .map((item) => item.name)
      .join("/");
...
const fetchSongUrl = async (id) => {
  if (!id) return;
  try {
    const res = await api.get("/song/url", { params: { id } });
    ...
    <div class="player-inner">
      ...
      <div class="player-control">
        <div class="control-main">
          <button class="btn-circle btn-large" 
